...
class QianwenService(BaseService):
    ...
    async def send_image(self, images, host_url, prompt, stream=False):
        """
        发送图片请求到通义千问视觉模型
        ...
        try:
            ...
            # 构建消息内容（展开列表）
            content_list = [{"type": "text", "text": prompt}]  # 文本部分
    ...
    async def send_message(self, message, stream=False, images=None, host_url=None, history=None,inter=False):
        """发送消息"""
        try:
            # 从配置文件加载参数
            config = load_config("qianwen")
            ...
            if images and len(images) > 0:
                ...
                # 调用视觉API
                return await self.send_image(images, host_url, message, stream=stream)
                        ...
                        if isinstance(msg['content'], str):
                            ...
                            for content_item in msg['content']:
                                if isinstance(content_item, dict):
                                    ...
                                    #    processed_image = await self._process_image(content_item['image'], host_url)
                                    #    processed_content.append({"image": processed_image})
